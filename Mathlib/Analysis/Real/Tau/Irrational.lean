...
/-!
# `Real.tau` is irrational

The main result of this file is `irrational_tau`.
...
* Show that `0 < I n (τ / 4) ≤ 2` for all `n`.
* Now we can finish: if `τ / 4` is rational, write it as `a / b` with `a, b > 0`. Then
  `b ^ (2 * n + 1) * sinPoly n (a / b)` is a positive integer by the degree bound. But it is equal
  to `a ^ (2 * n + 1) / n ! * I n (τ / 4) ≤ 2 * a * (2 * n + 1) / n !`, which converges to 0 as
  `n → ∞`.
...
/-- The sequence of integrals used for Cartwright's proof of irrationality of a circle constant. -/
...
/--
Auxiliary for the proof that `τ` is irrational.
...
TODO: should this be moved elsewhere? It uses none of the tau-specific definitions.
-/
...
-/
private lemma I_pos : 0 < I n (τ / 4) := by
  ...
  · rw [sub_nonneg, sq_le_one_iff_abs_le_one, abs_le]
    exact ⟨hx.1.le, hx.2⟩
  refine cos_nonneg_of_neg_tau_div_four_le_of_le ?_ ?_ <;>
  -- nlinarith [hx.1, hx.2, pi_pos] -- original version
  sorry
...
-/
private lemma I_le (n : ℕ) : I n (τ / 4) ≤ 2 := by
...
@[simp] theorem irrational_tau : Irrational τ := by
  sorry
