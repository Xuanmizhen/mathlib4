  ...
  · rw [Set.mem_Icc, not_and_or, not_le, not_le] at h₁
    ...
    · replace hθ := hθ.1
      have hcos : Real.cos θ < 0 := by
        ...
        refine Real.cos_pos_of_mem_Ioo_with_pi ⟨?_, ?_⟩ <;> linarith
    ...
    · replace hθ := hθ.2
      ...
      have hsin : 0 ≤ Real.sin θ := Real.sin_nonneg_of_mem_Icc_with_pi ⟨by linarith, hθ⟩
...
lemma arg_exp_mul_I (θ : ℝ) :
  ...
  · rw [← exp_mul_I, eq_sub_of_add_eq <| toIocMod_add_toIocDiv_zsmul _ _ θ, ofReal_sub,
      ofReal_zsmul, ofReal_mul, ofReal_ofNat, exp_mul_I_periodic_with_pi.sub_zsmul_eq]
...
theorem arg_nonneg_iff {z : ℂ} : 0 ≤ arg z ↔ 0 ≤ z.im := by
  ...
  calc
    0 ≤ arg z ↔ 0 ≤ Real.sin (arg z) :=
      ⟨fun h => Real.sin_nonneg_of_mem_Icc_with_pi ⟨h, arg_le_pi z⟩, by
        contrapose!
        intro h
