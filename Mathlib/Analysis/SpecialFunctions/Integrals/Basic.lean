...
theorem integral_sin_pow_succ_le : (∫ x in 0..π, sin x ^ (n + 1)) ≤ ∫ x in 0..π, sin x ^ n := by
  let H x h := pow_le_pow_of_le_one (sin_nonneg_of_mem_Icc_with_pi h) (sin_le_one x) (n.le_add_right 1)
...
theorem integral_sqrt_one_sub_sq : ∫ x in (-1 : ℝ)..1, √(1 - x ^ 2 : ℝ) = π / 2 :=
  calc
    ...
    _ = ∫ x in (-(π / 2))..(π / 2), cos x ^ 2 := by
          ...
          rw [← Real.cos_eq_sqrt_one_sub_sin_sq_with_pi (le_of_lt h.1) h.2, pow_two]
    _ = π / 2 := by simp
